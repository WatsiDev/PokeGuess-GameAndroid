...
import com.watsidev.myapplication.data.local.UserPreferencesRepository
...
import com.watsidev.myapplication.domain.usecase.ComparePokemonUseCase
        ...
        if (query.length >= 2) {
            searchJob = viewModelScope.launch {
                try {
                    ...
                    _uiState.update { it.copy(isSearching = false, error = "Search failed. Please try again.") }
                }
            }
        } else {
            _uiState.update { it.copy(searchResults = emptyList(), isSearching = false) }
